...
pred = model.inference(Xval);

rmseHF_hist(1) = sqrt(mean((pred-YH_val).^2));
...
fprintf('\nFinal RMSE = %.6f\n',rmse_hist(end))
    ...
    rmseHF_hist(n+1)=sqrt(mean((pred-YH_val).^2));
...
[x1,x2] = meshgrid(linspace(-1,1,ng));

Xtest = [x1(:) x2(:)];

Ytrue = f_H(Xtest);
Ytrue = reshape(Ytrue,ng,ng);

mu = model.inference(Xtest);

figure

surf(x1,x2,reshape(mu,ng,ng))

shading interp

hold on

scatter3(model.Xc(:,1),model.Xc(:,2),model.yc,...
    30,'filled')
...
surf(x1,x2,reshape(Ytrue,ng,ng))
...
title('True multi-fidelity surrogate')
