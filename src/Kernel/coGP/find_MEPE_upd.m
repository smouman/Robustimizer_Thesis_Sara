function x_new = find_MEPE(obj, bounds, count, HF, plot_graphs)
...
%% MEPE function 
fun = @(x) -( ...
    alpha * e_CV_nearest(obj, e_CV, X, x) + ...
...
if obj.dim == 1 && plot_graphs
    ...
    for i = 1:length(xgrid)
        ...
        E(i) = alpha * e_CV_nearest(obj, e_CV, X, x) + ...
               (1 - alpha) * obj.variance(x) ...
        ;

        V(i) = obj.variance(x);

        T(i) = e_CV_nearest(obj, e_CV, X, x);
...
function e_CV = e_CV_low(obj)

    % Make sure GP quantities are current
    obj.likelihood(obj.params);

    % K^{-1}
    invK = obj.L' \ (obj.L \ eye(obj.N));

    % Fast leave-one-out squared errors
    e_CV = (obj.alpha ./ diag(invK)).^2;

end



function e = e_CV_nearest(obj, e_CV, X, x)
% k = dsearchn(obj.Xe, x);
% e = e_CV(k);

idx = knnsearch(X, x, 'Distance', 'mahalanobis');
e = e_CV(idx);
